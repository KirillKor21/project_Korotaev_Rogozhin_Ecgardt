...
QByteArray cripto::chipr_data(QString data, unsigned char *key) {

    QByteArray data_bit = data.toUtf8(); // Переводим в байтовое значение

    unsigned char chipr[512];

    int data_len = encrypt((unsigned char *)data_bit.data(), data_bit.length(), key, iv, chipr); // Шифруем строчку с логином и паролем
    QByteArray chipr_data_bit =  QByteArray::fromRawData((const char*)chipr, data_len).toHex(); // Результат переводим в байтовое значение

    qDebug() << "chipr_data_bit = " + chipr_data_bit;

    return chipr_data_bit;
